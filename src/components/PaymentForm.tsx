...
export const PaymentForm: React.FC<PaymentFormProps> = ({ amount, onPaymentSuccess, onBack }) => {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      {isProcessing ? (
        <div className="w-full min-h-[350px] flex flex-col items-center justify-center py-12 text-center animate-fade-in">
          <Loader2 className="w-12 h-12 text-white animate-spin mb-6" />
          <h3 className="text-lg font-semibold text-white mb-2">Verifying Payment Screenshot</h3>
          <div className="h-1.5 w-56 bg-neutral-900 rounded-full overflow-hidden mb-4 border border-neutral-800">
            <div
              className="h-full bg-white transition-all duration-300"
              style={{ width: `${((processingStep + 1) / stepsTexts.length) * 100}%` }}
            />
          </div>
          <p className="text-sm text-zinc-400 font-medium animate-pulse">
            {stepsTexts[processingStep]}
          </p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6 animate-fade-in">
          ...
          <div className="flex gap-4 mt-2">
            ...
            <button
              type="submit"
              className="flex-1 py-3 px-6 rounded-lg bg-white hover:bg-neutral-200 active:scale-95 text-neutral-900 transition-all duration-200 text-sm font-bold flex items-center justify-center gap-2 cursor-pointer shadow-lg"
            >
